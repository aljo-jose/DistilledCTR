...
from distilled_ctr.layer import FactorizationMachine
...
class FeatureNN(torch.nn.Module):
    def __init__(self,
        ...
        self.layers.insert(0, shallow_layer(embed_dim, shallow_units)) # 1 to embed_dim
...
class NeuralAdditiveFactorizationMachines(torch.nn.Module):
    def __init__(self,
                 input_size: int,
                 field_dims,
                 embed_dim:int,
    ...
    def forward(self, x):
        ...
        f_out = torch.cat(self._feature_nns(embed_x), dim=-1)
        f_out = self.feature_dropout(f_out)
        nam_out = f_out.sum(axis=-1) + self.bias

        fm_out = self.fm(embed_x)
        
        output = torch.sigmoid(nam_out.squeeze(1) + fm_out.squeeze(1))
        return output

    def get_observation_weights(self, x):
        embed_x = self.embedding(x)
        f_out = torch.cat(self._feature_nns(embed_x), dim=-1)
        f_out = self.feature_dropout(f_out)
        f_out = torch.sigmoid(f_out.squeeze())
        return f_out
    
    def get_feature_weights(self, feature_index, input_indices):
        embed_x = self.embedding(input_indices)
        weights = self.feature_nns[feature_index](embed_x)
        return weights
